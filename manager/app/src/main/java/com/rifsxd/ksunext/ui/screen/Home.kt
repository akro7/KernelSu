...
fun HomeScreen(navigator: DestinationsNavigator) {
    ...
    Scaffold(
        ...
        Column(
            ...
            // ── AKRO Alpha: KernelPatch Integration Card ──
            KernelPatchCard()
...
@Composable
fun KernelPatchCard() {
    var magicMountHiding by remember {
        mutableStateOf(
            runCatching {
                Runtime.getRuntime().exec(arrayOf("su", "-c", "cat /proc/sys/kernel/kp_magic_mount_hiding 2>/dev/null || echo 0"))
                    .inputStream.bufferedReader().readLine()?.trim() == "1"
            }.getOrDefault(false)
        )
    }

    var rehookMode by remember {
        mutableStateOf(
            runCatching {
                Runtime.getRuntime().exec(arrayOf("su", "-c", "cat /proc/sys/kernel/kp_rehook_mode 2>/dev/null || echo 0"))
                    .inputStream.bufferedReader().readLine()?.trim() == "1"
            }.getOrDefault(false)
        )
    }
    ...
    AnimatedVisibility(
        ...
        ElevatedCard(
            ...
            colors = CardDefaults.elevatedCardColors(
                containerColor = androidx.compose.ui.graphics.Color(0xFF1A0008)
            ),
            ...
            Column(
                ...
                Row(
                    ...
                    Icon(
                        ...
                        tint = androidx.compose.ui.graphics.Color(0xFFDC143C),
                        modifier = Modifier
                            .size(28.dp)
                            .padding(end = 4.dp)
                    )
                    ...
                    Column {
                        Text(
                            ...
                            color = androidx.compose.ui.graphics.Color(0xFFDC143C)
                        )
                        Text(
                            text = "Kernel-level protection",
                            style = MaterialTheme.typography.bodySmall,
                    ...
                    Surface(
                        ...
                        color = androidx.compose.ui.graphics.Color(0xFF00E676).copy(alpha = 0.15f)
                    ) {
                        Text(
                            text = "● Active",
                ...
                HorizontalDivider(
                    color = androidx.compose.ui.graphics.Color(0xFF3A0010),
                    modifier = Modifier.padding(bottom = 12.dp)
                )

                // Magic Mount Hiding toggle
                KernelPatchToggleItem(
                    icon = Icons.Filled.VisibilityOff,
                    title = "Magic Mount Hiding",
                    subtitle = "Hide Magisk mounts from detection",
                    checked = magicMountHiding,
                    onCheckedChange = { newVal ->
                        magicMountHiding = newVal
                        val cmd = if (newVal) "1" else "0"
                        runCatching {
                            Runtime.getRuntime().exec(arrayOf("su", "-c",
                                "echo $cmd > /proc/sys/kernel/kp_magic_mount_hiding"))
                        }
                    }
                )
                ...
                KernelPatchToggleItem(
                    ...
                    title = "Rehook Mode",
                    subtitle = "Control syscall rehooking behavior",
                    checked = rehookMode,
                    onCheckedChange = { newVal ->
                        rehookMode = newVal
                        val cmd = if (newVal) "1" else "0"
                        runCatching {
                            Runtime.getRuntime().exec(arrayOf("su", "-c",
                                "echo $cmd > /proc/sys/kernel/kp_rehook_mode"))
                        }
                    }
                )
            }
...
@Composable
private fun KernelPatchToggleItem(
    ...
    Row(
        modifier = Modifier
            ...
            .background(androidx.compose.ui.graphics.Color(0xFF2A0A10))
        ...
        Icon(
            imageVector = icon,
            contentDescription = null,
            tint = if (checked) androidx.compose.ui.graphics.Color(0xFFDC143C)
        ...
        Switch(
            ...
            colors = SwitchDefaults.colors(
                ...
                checkedTrackColor = androidx.compose.ui.graphics.Color(0xFFDC143C),
                uncheckedThumbColor = androidx.compose.ui.graphics.Color(0xFF9E9E9E),
                uncheckedTrackColor = androidx.compose.ui.graphics.Color(0xFF2A0A10)
            )
        )
    }
}
...
private fun StatusCard(
    ...
    // ── AKRO Alpha: Kitsune-style animated StatusCard ──
    ...
    AnimatedVisibility(
        visible = cardVisible,
        enter = fadeIn(tween(500)) + slideInVertically(tween(500, easing = androidx.compose.animation.core.EaseOutCubic)) { it / 3 }
    ) {
    ElevatedCard(
        shape = RoundedCornerShape(24.dp),
        colors = CardDefaults.elevatedCardColors(containerColor = run {
            when {
                ksuVersion != null -> androidx.compose.ui.graphics.Color(0xFF9B0D24) // Deep Crimson
                kernelVersion.isGKI() -> MaterialTheme.colorScheme.secondaryContainer
